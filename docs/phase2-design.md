...
> 작성일: 2026-07-07 · 최초 기준 코드: `main` @ e4c3242
>
> **상태 (2026-07-13 갱신): 전체 구현됨. 단, 설계 당시와 구현 경로가 달라졌다.**
>
> 이 문서를 쓴 뒤 `main`이 37커밋 앞서 나가면서 **F2·F5·F7·F8을 먼저, 그리고 더
> 낫게 구현했다.** 그래서 최종 구현은 아래 설계를 그대로 따르지 않는다:
>
> | ID | 설계안 | 실제 |
> |---|---|---|
> | F2 증분 재스캔 | `rescan_subtree` + `SubtreeDelta` (하드링크 전역 정확도 포기) | **main의 `ScanHandle::rescan_paths`** — 영속 `HardlinkRegistry` + 증분으로 정확성을 보장 못 하면 풀스캔 강등. 설계안보다 강하다. |
> | F5 mtime/나이 | 신규 | main의 `modified_epoch` + `stale_files` |
> | F7 스냅샷 보존 | 신규 | main의 `prune_snapshots` |
> | F8 CLI 확장 | 신규 | main의 ncdu/dups/categories/advice export (`--suggest`만 F6용으로 추가) |
>
> F1·F3·F4·F6·F9만 새로 구현했고, 전부 main의 기존 API 위에 배선했다.
> 특히 **F1의 실행 후 검증은 `rescan_paths`의 `degraded` 플래그를 쓴다** — 풀스캔으로
> 강등되면 `~/.Trash`가 재계상돼 실측 회수량이 0에 가깝게 상쇄되므로, 그때는 0이
> 아니라 "측정 불가"로 보고한다. 아래 F2 절의 `refresh_paths`/`RefreshSummary`는
> **구현되지 않은 설계안**이다.
