...
fn find_git_dirs(root: &std::path::Path, out: &mut Vec<PathBuf>, depth: usize) {
    ...
    for e in rd.flatten() {
        ...
        if n == "node_modules" || n == "target" || n == ".build" {
            continue;
        }
        subdirs.push(p);
    }
...
fn main() {
    ...
    let t = t0.elapsed();
    println!(
        "발견 {} candidates → {} repos, {}개 조회성공 {}개 실패, {:.2}s",
        cands.len(),
        repos.len(),
        ok.len(),
        fail.len(),
    ...
    for r in &ok {
        *by_risk.entry(format!("{:?}", r.risk)).or_insert(0) += 1;
    }
    println!("위험도 분포: {:?}", by_risk);
    for r in ok.iter().take(8) {
        println!(
            "  [{:?}] {} dirty={} untracked={} stash={} ahead={:?} remote={} {}일전",
            r.risk,
